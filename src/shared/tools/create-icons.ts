...
const readFiles = async (): Promise<string[]> => {
	try {
		return fs.promises.readdir(directoryPath, { recursive: true }).then((files: string[]) => files.filter(file => file.endsWith('.svg')));
	} catch (err) {
...
const generateIconImportName = (iconName: string): string => {
	const name = iconName
		...
		.replace(/\s+/g, '')
		.replace('-', '');
	return `Icon_${name.charAt(0).toUpperCase() + name.slice(1)}`;
};
