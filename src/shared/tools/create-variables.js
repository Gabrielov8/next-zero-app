...
const jsonFiles = fs.readdirSync(`./src/shared/ui/theme/variables`).map(file => file.split('.')[0]);
...
for (const type of jsonFiles) {
	...
	try {
		...
		fs.writeFileSync(path.join(`./src/shared/ui/theme/scss/${type}/${type}.constants.ts`), resultTs);
		resultSass = '';
		resultTs = '';
		console.log('\x1b[32m', type.charAt(0).toUpperCase() + type.substring(1) + ' variables successfully updated!');
	} catch (e) {
